...
while True:
    ...
    halflength = 50
    x_start = max(0, x - (2*halflength))
    y_start = max(0, y - halflength)
    x_end = min(frame1.shape[1], x + (2*halflength))
    ...
    try:
        ...
        objectDistance = round(175.21*(1.0165**(pixelDistance))) # todo: update equation for object distance upto 2 m at 10cm intervals
        ...
        text = f'closest object at {objectDistance} centimeters' 
        # print(text)
